...
const Project = () => {
  ...
  const handleAction = async (id, e, type) => {
    ...
    if (type === 'permanent_delete') {
        if (!window.confirm("คำเตือน: การลบถาวรจะไม่สามารถกู้คืนข้อมูลได้อีก คุณแน่ใจหรือไม่?")) {
            return;
        }
    }
    ...
    try {
      ...
      alert("การทำงานล้มเหลว กรุณาลองใหม่อีกครั้ง");
    }
  };

  return (
    <div className="p-5 h-full flex flex-col overflow-hidden">
      {/* --- Header & Tabs (ส่วนนี้เหมือนเดิม) --- */}
            ...
            {isLoading ? (
              ...
              projects.map((item, index) => (
                <ProjectCard 
                  ...
                  index={index} 
                  mode={activeTab} 
                  onAction={(e, type) => handleAction(item.id, e, type)} 
                />
              ))
            ) : (
...
const ProjectCard = ({ data, index, mode, onAction }) => {
  return (
    <motion.div
      layout
      ...
      className="bg-[#FBCDCB] h-32 shrink-0 rounded-[35px] shadow-sm flex items-center px-10 justify-between cursor-pointer group hover:bg-[#F9BCB9] transition-all border border-transparent hover:border-white/30"
    >
      <div>
      ...
      <div className="flex items-center gap-3">
        {/* --- ปุ่ม Action ตัวแรก (Soft Delete / Restore) --- */}
        {mode === 'all' ? (
          <button 
        ...
        {/* --- ปุ่ม Action ตัวที่สอง (ลูกศร / Permanent Delete) --- */}
        {mode === 'all' ? (
